...
if os.path.exists("./train/modelBackUp_best_inPrediction.h5"):
    opponentRl.load_backup_model("./train/modelBackUp_best_inPrediction.h5")
reward = 0
...
while running:
    ...
    pygame.draw.line(window, config.line_color, (config.line_start_x_left, center_y-(config.width_of_lines/2)), (config.line_end_x_left, center_y+(config.width_of_lines/2)), config.line_thick)  # line3 left of window
    ...
    # referee part
    quitGame = functions.referee(center_y, center_y, y_opponent, edgeState)
    if(quitGame & (player2_score == config.finalScore or player1_score == config.finalScore) ):
        running = False
    else:
        if(edgeState == 'left' and  quitGame):
            player2_score += 1
        elif(edgeState == 'right' and quitGame):
            player1_score += 1
